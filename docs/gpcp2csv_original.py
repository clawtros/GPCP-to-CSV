...
class GpcpParser:
    ...
    def generate_map(self):
        ...
        start = LatLon(*[Coord(p) for p in pstr[pstr.index('(')+1:pstr.index(')')].split(',')])

        pstr = pstr[pstr.index('2nd_box_center'):]
        next = LatLon(*[Coord(p) for p in pstr[pstr.index('(')+1:pstr.index(')')].split(',')])

        pstr = pstr[pstr.index('last_box_center'):]
        end = LatLon(*[Coord(p) for p in pstr[pstr.index('(')+1:pstr.index(')')].split(',')])
